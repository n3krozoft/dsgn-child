...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'post portfolio-column' ); ?>>
	...
	<header class="entry-header">
		...
		// better: define taxonomies
		
		echo '<div class="entry-content">';
		
		// get the year:
		
		echo '<span class="portfolio-entry-meta">';
				echo get_the_time('Y');
		echo ' &mdash; </span>';
		...
		echo '<span class="portfolio-entry-meta">';
			...
			echo get_modified_term_list( get_the_ID(), 'category', '', ', ', '', array(3,8) );
			
		
		echo '</span></div>';
		// echo get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '<div class="entry-content"><span class="portfolio-entry-meta">', _x(', ', '', 'designer' ), '</span></div>' ); 
